...
async def list_schemes(session: SessionDep, tenant: Tenant = Depends(get_current_tenant)):
...
async def create_scheme(request: PricingSchemeCreate, session: SessionDep, tenant: Tenant = Depends(get_current_tenant)):
    """Create a new pricing scheme (e.g., standard, premium tier)."""
    # Verify URN is unique for the tenant
    result = await session.execute(select(PricingScheme).where(PricingScheme.urn == request.urn, PricingScheme.tenant_id == tenant.id))
    if result.scalars().first():
        raise HTTPException(status_code=400, detail="A scheme with this URN already exists.")
...
async def add_rule_version(scheme_id: UUID, request: RuleVersionCreate, session: SessionDep, tenant: Tenant = Depends(get_current_tenant)):
    """Adds a new deterministic math rule version to a scheme."""
    
    # 1. Validate scheme belongs to tenant
    result = await session.execute(select(PricingScheme).where(PricingScheme.id == scheme_id, PricingScheme.tenant_id == tenant.id))
