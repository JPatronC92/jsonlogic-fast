...
class PricingEngine:
    ...
    def simulate_batch(self, transacciones: List[Dict[str, Any]], reglas_activas: List[PricingRuleVersion]) -> BatchSimulateResponse:
        """
    ...
    def calculate(self, contexto_tx: Dict[str, Any], reglas_activas: List[PricingRuleVersion]) -> CalculateFeeResponse:
        calculated_fees = []
        ...
        for regla_version in reglas_activas:
            if regla_version.context_schema:
                try:
                    validate(instance=contexto_tx, schema=regla_version.context_schema.schema_json)
                except ValidationError as e:
                    logger.error(f"Contexto inválido para regla {regla_version.rule.name}: {e.message}")
                    raise ValueError(f"Payload malformado para {regla_version.rule.name}: {e.message}")
        ...
        for regla_version in reglas_activas:
            try:
                ...
                logger.error(f"Error ejecutando json-logic para rule {regla_version.rule_uuid}: {e}")
                raise ValueError(f"Error calculando fee para {regla_version.rule.name}: {str(e)}")
